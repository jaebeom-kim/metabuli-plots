...
def distribution() -> None:
    ...
    gtdb_species_fp = pd.read_csv('./prokaryote/strain_ont_1110_classifications.tsv.species.fp', sep='\t', header=None)
    ...
    virus_genus_tp = pd.read_csv('./virus/genus_ont_1120_classifications.tsv.genus.tp', sep='\t', header=None)
    virus_genus_fp = pd.read_csv('./virus/genus_ont_1120_classifications.tsv.genus.fp', sep='\t', header=None)
    virus_species_tp = pd.read_csv('./virus/strain_ont_1120_classifications.tsv.species.tp', sep='\t', header=None)
    virus_species_fp = pd.read_csv('./virus/strain_ont_1120_classifications.tsv.species.fp', sep='\t', header=None)
    ...
    y_ranges = [[70000, 2000, 70000, 500],
                [70000, 2000, 70000, 500]]
    ...
    for i in range(2):
        for j in range(4):
            ...
            axs[i, j].text((min_score + 1)/2, 0.8, f'{100 * (data[i][j] > 0.008).sum() / len(data[i][j]):.1f}%',
    ...
    # Save figure
    plt.savefig('ont_score.png', dpi=300, bbox_inches='tight', transparent=True)
